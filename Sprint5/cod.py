#This is the final Certificate of Deposit (COD) interface, it purpose is to replicate a COD account in a bank
...
#imports user data to control each user account
...
class Investment:
    ...
    #A function to change status of finished stocks
    def is_finsihedstocks(self):
        return self.progress() >= 1.0

# A CertificateOfDepoist class for the COD investment# A CertificateOfDepoist class for the COD investment
class CertificateofDepositApp:
    ...
    def add_investment(self):
        ...
        #Updates the history of accountbalance for current user
    ...
    # a function to check if the compounding of investments is finished
    def processfinished(self):
        ...
        #For loops for finished investments, returns money back to main portfolio and removes investment from growing investment for current user
        ...
        #updates data and history, saves the updates
        self.controller.frequentdatarefresh()
        self.controller.users_data[self.controller.current_user_token]["history"].append((datetime.now().strftime("%Y-%m-%d %H:%M:%S"), self.controller.totalaccountbalance()))
        save_users(self.controller.users_data)
        return True
    ...
    def calculate_returns(self):
        ...
        self.returns_calculated = True
        self.calculate_button.configure(state="disabled", text="Returns Calculated")

    # A function to remove investment from portfolio
    # Current user enters name and it gets removed from portfolio based on lowercase name
    ...
    def menuGui(self):
        ...
        customtkinter.CTkLabel(left_frame, font=("Banschrift", 20), text="Investment Name").grid(row=0, column=0, padx=20, pady=5, sticky="w")
        self.entry_name = customtkinter.CTkEntry(left_frame)
        self.entry_name.grid(row=1, column=0, padx=20, pady=5, sticky="ew")

        customtkinter.CTkLabel(left_frame, font=("Banschrift", 20), text="Amount").grid(row=2, column=0, padx=20, pady=5, sticky="w")
        ...
        customtkinter.CTkLabel(left_frame, font=("Banschrift", 20), text="Time Periods").grid(row=6, column=0, padx=20, pady=5, sticky="w")
        row = 0
        column = 0
        ...
        customtkinter.CTkLabel(left_frame, font=("Banschrift", 20), text="Remove an Investment").grid(row=14, column=0, padx=20, pady=(20, 5), sticky="w")
        ...
        customtkinter.CTkLabel(rightbottom_frame, font=("Banschrift", 20), text="Calculate Return On Investments").grid(row=0, column=0, padx=20, pady=5, sticky="w")
        ...
        self.returns = customtkinter.CTkLabel(rightbottom_frame, font=("Banschrift", 35), text="")
        ...
        returnback_button.place(x=650, y=735)
        #Runs verify COD
        self.verify_cod()